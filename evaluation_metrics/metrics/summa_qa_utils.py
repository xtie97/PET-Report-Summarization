...
class QA_Metric:
    ...
    def __init__(self, model=None, batch_size=8, max_seq_len=512, use_gpu=True):
    ...
    def compute(self, questions, true_asws, evaluated_text):
        ...
        for count, (question, true_asw) in enumerate(zip(questions, true_asws)):
            if count % self.batch_size == 0 and count != 0:
                ...
                probs.extend(prob)
                slines = []
            cur_dict = self.model.tokenizer.encode_plus(question, evaluated_text, max_length=self.max_seq_len, padding='max_length', truncation=True, return_token_type_ids=True)
...
def evaluate_corpus(srcs, gens, model=None, questionss=None, aswss=None, batch_size=8, max_seq_len=512, aggregate=True):
    """
