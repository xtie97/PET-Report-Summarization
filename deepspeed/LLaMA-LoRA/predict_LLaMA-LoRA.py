...
def predict_text(reports, lora_weights, savename, test_epoch): 
    # load model 
    model = AutoModelForCausalLM.from_pretrained("llama-7B", # local folder to save the LLaMA-7B weights 
                                                load_in_8bit=False, 
                                                torch_dtype=torch.half, # torch.float32 full precision inference/torch.half: half precision inference
                                                device_map="auto") # original LLAMA-7B model
    ...
    tokenizer = AutoTokenizer.from_pretrained("my-llama-7B", padding_side="left", truncation_side="right") # add special tokens
...
if __name__ == '__main__':
    ...
    print('Start testing')
    lora_weights = f"my-llama-7B/checkpoint-5038" 
    predict_text(df, lora_weights, "llama-7b-lora", test_epoch=17)
