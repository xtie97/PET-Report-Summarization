...
def parse_args():
    ...
    parser.add_argument("--model_id", type=str, default="gpt2-large", help="Model id to use for training.")
    ...
    parser.add_argument("--gradient_checkpointing", type=bool, default=True, help="Saving memory using gradient-checkpointing.")
    parser.add_argument(
        "--bf16",
        type=bool,
...
def training_function(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args.model_id,
        pad_token_id=tokenizer.pad_token_id,  # for LLaMA
        use_cache=False if args.gradient_checkpointing else True,  # this is needed for gradient checkpointing
    )

    #Creating config corresponding to the PEFT method (hyper-param from radiology-GPT)
    config = LoraConfig(
        r=8,
        lora_alpha=16,
    ...
    gradient_accumulation_steps = 128 // torch.cuda.device_count() // args.per_device_train_batch_size # 128 from radiology-GPT
