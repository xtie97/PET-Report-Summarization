...
def parse_args():
    ...
    parser.add_argument("--model_id", type=str, default="gpt2-xl", help="Model id to use for training.")
...
def training_function(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.model_id, padding_side="right", truncation_side="right")
    #tokenizer.pad_token = tokenizer.eos_token  # only for GPT2
    #tokenizer.pad_token_id = tokenizer.eos_token_id # only for GPT2 
    tokenizer.bos_token = DEFAULT_BOS_TOKEN # only for OPT / LLAMA
    tokenizer.eos_token = DEFAULT_EOS_TOKEN # only for OPT / LLAMA
    #tokenizer.pad_token = DEFAULT_PAD_TOKEN # OPT has the default pad token
    tokenizer.unk_token = DEFAULT_UNK_TOKEN # only for OPT / LLAMA
    ...
    data_collator = DataCollatorForSeq2Seq(
        tokenizer, model=model, label_pad_token_id=-100, pad_to_multiple_of=8, padding=True, return_tensors="pt"
    )
