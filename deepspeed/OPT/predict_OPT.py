...
import deepspeed
# Command: deepspeed --num_gpus=2 predict_OPT.py
...
def predict_text(reports, model_template, savename, test_epoch):
    tokenizer = AutoTokenizer.from_pretrained(model_template.split("/")[0], padding_side="left", truncation_side="right") # "left" for batch generation 
    ...
    ds_model = deepspeed.init_inference(
        ...
        max_out_tokens=2048, # max output length
        mp_size=num_GPU,        # Number of GPU
        dtype=torch.float32, # torch.half: dtype of the weights (fp16), torch.float32: dtype of the weights (fp32)
    ...
    encoder_max_length = 2048 - decoder_max_length # 2048 is the max length in OPT 
...
if __name__ == '__main__':
    ...
    print('Start testing')
    model_template = f"my-opt-1.3b/checkpoint-12594" 
    predict_text(df, model_template, 'opt-1.3b', test_epoch=12)
